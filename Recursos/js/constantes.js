// Sección de constantes compartidas por todas las páginas
const TITULO_PAGINA = "Mi Sitio Web";
const DERECHOS_RESERVADOS = "© 2023 Todos los derechos reservados.";
const POLITICA_PRIVACIDAD = "Política de Privacidad";
const TERMINOS_CONDICIONES = "Términos y Condiciones";

// Sección de código para la página "index"
if (document.title === "Inicio") {
  // Código específico para la página de inicio
  const TITULO_PAGINA = "Página de Inicio";
  const DESCRIPCION_PAGINA = "Bienvenido a la página de inicio";
}

// Sección de código para la página "informacion"
if (document.title === "Ejercicios") {
}